use {
    ...
    carbon_raydium_amm_v4_decoder::{
        instructions::{swap_base_in::SwapBaseIn, swap_base_out::SwapBaseOut, RaydiumAmmV4Instruction}, RaydiumAmmV4Decoder, PROGRAM_ID as RAY_V4_PROGRAM_ID
    },
    carbon_yellowstone_grpc_datasource::YellowstoneGrpcGeyserClient,
    once_cell::sync::Lazy,
    raydium_amm_monitor::{
        config::{
            init_jito, init_nozomi, init_zslot, BUY_SOL_AMOUNT, CONFIRM_SERVICE, JITO_CLIENT, NOZOMI_CLIENT, POOL_ADDRESS, PRIORITY_FEE, PUBKEY, RPC_CLIENT, SLIPPAGE, ZSLOT_CLIENT
        },
    ...
    tokio::{sync::RwLock, time::{interval, sleep}},
...
pub async fn main() -> CarbonResult<()> {
    ...
    println!("Purchase Amount: {}", *BUY_SOL_AMOUNT);
...
fn display_pool_price_change(old: f64, new: f64) {
    if old > 0.0 {
        ...
        if percent <= -10.0 {
            println!("ALERT: POOL_PRICE dropped more than 10%!");
        }
    }
}
...
impl Processor for RaydiumV4Process {
    ...
    async fn process(
        ...
        let raw_instruction: Vec<Instruction> = match instruction.data {
                    ...
                    SwapBaseIn::arrange_accounts(&instruction_clone.accounts)
                {
                    ...
                    if let (Some(coin_info), Some(pc_info), Some(pre_coin_info), Some(pre_pc_info)) = (coin_raw_info, pc_raw_info, pre_coin_raw_info, pre_pc_raw_info) {
                        ...
                        if input_mint == WSOL {
                            ...
                            let pool_price_sol = (post_input_reserve_val / 10f64.powf(9 as f64)) / (post_output_reserve_val / 10f64.powf(mint_decimal as f64));
                            ...
                            let pool_price_sol = (post_output_reserve_val / 10f64.powf(9 as f64)) / (post_input_reserve_val / 10f64.powf(mint_decimal as f64));
            ...
            // RaydiumAmmV4Instruction::SwapBaseOut(swap_base_out_data) => {
            ...
            //     if let Some(mut arranged) =
            //         SwapBaseOut::arrange_accounts(&instruction_clone.accounts)
            //     {
            //         let post_token_balance = metadata
            //             .transaction_metadata
            //             .meta
            ...
            //         // println!(
            //         //     "Coin Mint: {:?}, PC Mint: {:?}, Pre Coin Mint: {:?}, Pre PC Mint: {:?}",
            //         //     coin_raw_info, pc_raw_info, pre_coin_raw_info, pre_pc_raw_info
            //         // );
            ...
            //                 .expect("Invalid pre_input_reserve");
            //             // println!(
            //             //     "Post Input Reserve: {}, Post Output Reserve: {}, Pre Input Reserve: {}, Pre Output Reserve: {}",
            //             //     post_input_reserve_val, post_output_reserve_val, pre_input_reserve_val, pre_output_reserve_val
            //             // );
            //             let input_change = post_input_reserve_val - pre_input_reserve_val;
            //             let output_change = post_output_reserve_val - pre_output_reserve_val;
            ...
            //             let amount_out =  0.997 * (1.0 - *SLIPPAGE) * (amount_in as f64) * output_reserve_val
            //                     / (input_reserve_val + amount_in as f64);

            //             let buy_exact_in_param = SwapBaseOut {
            ...
            //             let swap_ix = arranged.get_swap_base_out_ix(buy_exact_in_param.clone());
            //             ix.push(swap_ix);
